...
static void mavlink_test_tuned_frequency(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
	mavlink_message_t msg;
        uint8_t buffer[MAVLINK_MAX_PACKET_LEN];
        uint16_t i;
	...
	mavlink_tuned_frequency_t packet1, packet2;
        ...
        memset(&packet2, 0, sizeof(packet2));
	mavlink_msg_tuned_frequency_encode(system_id, component_id, &msg, &packet1);
...
static void mavlink_test_rf_gain(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
	mavlink_message_t msg;
        uint8_t buffer[MAVLINK_MAX_PACKET_LEN];
        uint16_t i;
	mavlink_rf_gain_t packet_in = {
		17.0,17
    };
	mavlink_rf_gain_t packet1, packet2;
        memset(&packet1, 0, sizeof(packet1));
        ...
        memset(&packet2, 0, sizeof(packet2));
	mavlink_msg_rf_gain_encode(system_id, component_id, &msg, &packet1);
	mavlink_msg_rf_gain_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
	mavlink_msg_rf_gain_pack(system_id, component_id, &msg , packet1.target_system , packet1.gain );
	mavlink_msg_rf_gain_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
	mavlink_msg_rf_gain_pack_chan(system_id, component_id, MAVLINK_COMM_0, &msg , packet1.target_system , packet1.gain );
	mavlink_msg_rf_gain_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
	...
	mavlink_msg_rf_gain_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
        
        memset(&packet2, 0, sizeof(packet2));
	mavlink_msg_rf_gain_send(MAVLINK_COMM_1 , packet1.target_system , packet1.gain );
	mavlink_msg_rf_gain_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
}
...
static void mavlink_test_ardupilotmega(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
	...
	mavlink_test_rf_gain(system_id, component_id, last_msg);
